...
with app.app_context():
    ...
    def register():
        ...
        if register_form.validate_on_submit():
            if User.query.filter_by(user_email=register_form.email.data).first():
                flash("You have already signed up with that email, log in instead!")
                return redirect(url_for('login'))
            else:
                hash_pass = generate_password_hash(register_form.password.data, method="pbkdf2:sha256", salt_length=8)
                new_user = User(user_name=register_form.name.data,
    ...
    def login():
        ...
        if login_form.validate_on_submit():
            ...
            if not user:
                ...
                return redirect(url_for('login'))
            elif not check_password_hash(user.user_pass, login_form.password.data):
